...
async function main() {
  ...
  // AI Opportunities demo (5-10)
  const opps = [
    { slug:'woocommerce-connector', title:'WooCommerce connector', category:'Integration', valueScore:80, riskLevel:'Medium' },
    { slug:'timesheet-mobile-capture', title:'Timesheet mobile capture', category:'Feature', valueScore:75, riskLevel:'Low' },
    { slug:'ocr-vendor-b-multicurrency', title:'OCR vendor B (multi‑currency)', category:'Integration', valueScore:70, riskLevel:'Medium' },
    { slug:'powerbi-push-dataset', title:'Power BI push dataset', category:'Integration', valueScore:65, riskLevel:'Low' },
    { slug:'bank-auto-match-tuning', title:'Bank feed auto‑match tuning', category:'Feature', valueScore:60, riskLevel:'Low' },
    { slug:'office-addin-merge', title:'Office add‑in for document merge', category:'Feature', valueScore:55, riskLevel:'Low' },
  ];
  for (const it of opps) {
    await prisma.opportunity.upsert({ where:{ slug: it.slug }, update: { title: it.title, category: it.category, valueScore: it.valueScore, riskLevel: it.riskLevel }, create: { ...it } });
  }
