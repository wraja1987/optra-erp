"use client"
import ComingSoonBadge from '../../components/ui/ComingSoonBadge'

export default function OpenBankingPage() {
  const rows = [
    { date: '2025-08-20', desc: 'Coffee', amount: -3.2 },
    { date: '2025-08-21', desc: 'Supplier Payment', amount: -420.0 },
    { date: '2025-08-22', desc: 'Customer Receipt', amount: 1200.0 },
  ]
  const exportCsv = () => {
    const header = 'date,description,amount\n'
    const body = rows.map(r=>`${r.date},${r.desc},${r.amount}`).join('\n')
    const blob = new Blob([header + body], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'bank-feed-demo.csv'; a.click(); URL.revokeObjectURL(url);
  }
  return (
    <main role="main" style={{ padding: 16 }}>
      <h1 style={{ display:'flex', alignItems:'center', gap:8 }}>Open Banking <ComingSoonBadge /></h1>
      <p>Bank feeds and reconciliation preview with example transactions. All actions are mocked.</p>
      <div role="status" aria-live="polite" style={{ background:'#fff6d6', padding:12, borderRadius:8, margin:'12px 0' }}>
        This module is coming soon. Live features will include automatic feeds and matching.
      </div>
      <button disabled title="Mocked">Connect Bank (Mocked)</button>
      <div style={{ overflowX:'auto', marginTop:12 }}>
        <table role="table" aria-label="Bank feed preview" style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead><tr><th>Date</th><th>Description</th><th>Amount</th></tr></thead>
          <tbody>
            {rows.map((r,i)=> (
              <tr key={i}><td>{r.date}</td><td>{r.desc}</td><td>£{r.amount}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
      <div style={{ marginTop:12 }}>
        <button onClick={exportCsv}>Export CSV</button>
      </div>
      <section aria-labelledby="ai-help" style={{ marginTop:16, padding:12, border:'1px solid #eaecef', borderRadius:8 }}>
        <h2 id="ai-help">What Nexa AI will do here</h2>
        <ul>
          <li>Suggest bank rules to auto-categorise transactions.</li>
          <li>Flag unusual payments and propose approvals.</li>
          <li>Draft reconciliation summaries for month-end.</li>
        </ul>
      </section>
    </main>
  )
}



