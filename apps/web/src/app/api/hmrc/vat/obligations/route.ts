...
export async function GET() {
  ...
  audit({ route: '/api/hmrc/vat/obligations', ip: '0.0.0.0', session: 'hmrc' })
  if (!configured) return NextResponse.json({ ok: false, status: 'not_configured' }, { status: 200 })
  return NextResponse.json({ ok: true, status: 'active', data: [{ periodKey: '24A1', status: 'O', due: new Date().toISOString() }] })
}
