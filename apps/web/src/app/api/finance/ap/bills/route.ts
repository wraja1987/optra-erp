...
const CreateBody = z.object({ tenantId: z.string().min(1), number: z.string().min(1), supplierId: z.string().min(1), currency: z.string().default('GBP'), total: z.coerce.number().nonnegative() })
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const items = await prisma.supplierBill.findMany({ where: { tenantId } })
    return NextResponse.json({ ok: true, items, ...corr })
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Bad Request'
    audit({ route: '/api/finance/ap/bills', module: 'finance', action: 'error', message })
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const created = await prisma.supplierBill.create({ data: body })
    audit({ route: '/api/finance/ap/bills', module: 'finance', action: 'AP_BILL_CREATE', status: 'ok', number: created.number })
    return NextResponse.json({ ok: true, item: created, ...corr })
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Bad Request'
    audit({ route: '/api/finance/ap/bills', module: 'finance', action: 'AP_BILL_CREATE', status: 'error', message })
    return NextResponse.json({ ok: false, code: 'bad_request', message, ...corr }, { status: 400 })
  }
}
